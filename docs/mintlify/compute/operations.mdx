...
- Metrics saved to `artifacts/runs//metrics.json`.
...
- **Logs:** Check logs under `logs/` and `artifacts/runs//`.
...
3. Evaluate: `python -m nexa_eval.judge --checkpoint `
4. Package: `python scripts/cli.py package --checkpoint `
