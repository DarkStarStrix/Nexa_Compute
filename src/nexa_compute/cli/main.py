...
# Adjust ROOT to point to the repository root
# This file is in src/nexa_compute/cli/main.py (3 levels deep from src, 4 from root)
ROOT = Path(__file__).resolve().parents[3]
SRC = ROOT / "src"

# Ensure repo root and src are in sys.path to import core modules (nexa_data, etc.)
...
def leaderboard(host: Optional[str] = typer.Option(None, help="Hostname to bind UI"), port: int = typer.Option(8501)) -> None:
    ...
    # Adjusted path for nexa_ui since ROOT is now correctly resolved
    script_path = ROOT / "nexa_ui" / "leaderboard.py"
    cmd = [
