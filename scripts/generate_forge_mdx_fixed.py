...
def fix_code_blocks(content):
    # Add language tags where missing, keep existing tags untouched
    def repl(match):
        code = match.group(1)
        # Detect language heuristically
        if re.search(r'\bdef\b|\bimport\b|\bclass\b', code):
            lang = 'python'
        elif re.search(r'curl|#!/bin/bash', code):
            lang = 'bash'
        else:
            lang = 'text'
        return f'```{lang}\n{code}\n```'
    # Replace untagged fences ``````
...
def create_overview():
    ...
    # Remove any implementation/spec sections
